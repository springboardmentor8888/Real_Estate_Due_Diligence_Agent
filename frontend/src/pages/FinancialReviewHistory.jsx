  ...
  const fetchHistory = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/purchases/financial/history`,
        {
          headers: getHeaders(),
        },
      );
      ...
      console.error("Error loading financial review history:", error);

      setError(
        error.response?.data?.message ||
          "Unable to load financial review history.",
      );
    ...
    <div className="px-8 pt-6 pb-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-500">
          View previously approved and rejected financial reviews.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border shadow-sm">
        <div className="p-6 border-b">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Financial decisions made for buyer purchase requests.
          </p>
        </div>

        {loading ? (
          <div className="p-10 text-center text-gray-500">
            Loading financial review history...
          </div>
        ) : reviews.length === 0 ? (
          <div className="p-10 text-center">
            <p className="text-gray-500">
              No completed financial reviews found.
            </p>
          </div>
        ) : (
          <div className="divide-y">
            {reviews.map((review) => {
              const approved = review.financialStatus === "APPROVED";
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
                    ...
                    <Info
                      label="Reviewed By"
                      value={review.financialReviewerName || "Not available"}
                    />

                    <Info
                      label="Reviewed On"
                      value={formatDate(review.financialReviewedAt)}
                    />
                  </div>
...
export default FinancialReviewHistory;
