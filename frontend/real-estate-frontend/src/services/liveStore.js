...
export const getLiveActiveProperty = (idFromUrl) => {
  ...
  if (!targetId && allProps.length > 0) {
    targetId = allProps[0].numericId || allProps[0].id || 1001;
  }

  const cleanNumeric = parseInt((targetId || "1001").toString().replace(/\D/g, "") || "1001", 10);
  ...
  const found =
    ...
    ) || allProps[0] || DEFAULT_FALLBACK_PROPERTY;

  if (typeof window !== "undefined" && found) {
    const numId = (found.numericId || found.propertyId || 1001).toString();
    if (localStorage.getItem(KEYS.ACTIVE_PROPERTY_ID) !== numId) {
      localStorage.setItem(KEYS.ACTIVE_PROPERTY_ID, numId);
    }
  }

  return found || DEFAULT_FALLBACK_PROPERTY;
};

export const setLiveActiveProperty = (propertyOrId) => {
  if (!propertyOrId) return;
  const id = typeof propertyOrId === "object" ? (propertyOrId.numericId || propertyOrId.propertyId || propertyOrId.id) : propertyOrId;
  const cleanNumeric = (id || "1001").toString().replace(/\D/g, "") || "1001";
  if (typeof window !== "undefined") {
    localStorage.setItem(KEYS.ACTIVE_PROPERTY_ID, cleanNumeric);
    notifyLiveUpdate();
  }
};
