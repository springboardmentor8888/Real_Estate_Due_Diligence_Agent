...
function PropertyDetails() {
  ...
  const activeProp = getLiveActiveProperty(searchParams.get("id") || searchParams.get("propertyId") || location.state?.property?.id);
  const targetId = activeProp ? (activeProp.numericId || activeProp.propertyId || activeProp.id) : "1001";
  ...
  useEffect(() => {
    ...
    getPropertyDetails(cleanId)
      .then((res) => {
        if (res && res.data) {
          ...
          const propObj = {
            id: p.id || `PR-${p.propertyId || p.numericId || "1001"}`,
            propertyId: p.propertyId || p.numericId || 1001,
            numericId: p.numericId || p.propertyId || 1001,
            ...
            owner: p.ownerName || p.owner || "Adani Realty Institutional Fund",
            ownerName: p.ownerName || p.owner || "Adani Realty Institutional Fund",
    ...
    getOwnershipRecords(cleanId).then((res) => res?.data && setOwnershipRecords(Array.isArray(res.data) ? res.data : [res.data])).catch(() => {});
        ...
        <div className="space-y-6">
          ...
          {activeTab === "ownership" && (
            <div className="white-card rounded-3xl p-6 bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-[#334155] shadow-xs space-y-4 font-mono text-xs">
              <h3 className="text-sm font-extrabold text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                <User size={16} className="text-purple-500" /> 30-Year Sub-Registrar Title Deed Chain
              </h3>

              <div className="divide-y divide-slate-100 dark:divide-[#334155]">
                {[
                  { year: "2021 - Present", owner: p.ownerName, deed: p.deedNumber, status: "Verified Clear Title" },
                  { year: "2010 - 2021", owner: "Devi Infrastructure Projects Ltd", deed: "DEED/TS/2010/3312", status: "Verified Encumbrance Free" },
                  { year: "1996 - 2010", owner: "Telangana State Industrial Infrastructure Corp", deed: "DEED/HYD/1996/1044", status: "Govt Allotment Clear" },
                ].map((rec, idx) => (
                  <div key={idx} className="py-3 flex items-center justify-between gap-4">
                    <div>
                      <strong className="text-slate-900 dark:text-white font-extrabold block text-sm">{rec.owner}</strong>
                      <span className="text-slate-400">{rec.year} • Deed #{rec.deed}</span>
                    </div>
            ...
            <div className="white-card rounded-3xl p-6 bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-[#334155] shadow-xs space-y-4 font-mono text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800">
                <div>
                  ...
                  <strong className="text-emerald-700 dark:text-emerald-300 font-extrabold text-sm block mt-1">Zero Outstanding Liens</strong>
                </div>
                <div>
                  <span className="text-emerald-900 dark:text-emerald-300 uppercase text-[10px] font-bold">Latest Receipt No</span>
                  <strong className="text-slate-900 dark:text-white font-extrabold text-xs block mt-1">GHMC/TAX/2026/88102</strong>
                </div>
                <div>
                  <span className="text-emerald-900 dark:text-emerald-300 uppercase text-[10px] font-bold">Assessment Year</span>
                  <strong className="text-slate-900 dark:text-white font-extrabold text-xs block mt-1">2025 - 2026 Paid</strong>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === "risk" && (
            <div className="white-card rounded-3xl p-6 bg-white dark:bg-[#1E293B] border border-slate-200 dark:border-[#334155] shadow-xs space-y-4 font-mono text-xs">
              <h3 className="text-sm font-extrabold text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                <ShieldCheck size={16} className="text-blue-500" /> 13-Vector Due Diligence Risk Score Breakdown
              </h3>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  { vector: "1. Sub-Registrar Title Deed Search", score: "Clean (0/100)" },
                  { vector: "2. Encumbrance & Litigation Check", score: "Clean (0/100)" },
                  { vector: "3. Municipal Property Tax Liens", score: "Paid (0/100)" },
                  { vector: "4. HMDA Zoning & Master Plan FAR", score: "Verified Commercial" },
                  { vector: "5. FIRM Elevation & Flood Risk", score: "Zone X (Low Risk)" },
                  { vector: "6. Environmental NOC Clearance", score: "PCB NOC Active" },
                ].map((item, idx) => (
                  <div key={idx} className="p-3 rounded-xl bg-slate-50 dark:bg-[#0F172A] border border-slate-200 dark:border-[#334155] flex items-center justify-between">
                    <span className="text-slate-800 dark:text-slate-200 font-bold">{item.vector}</span>
