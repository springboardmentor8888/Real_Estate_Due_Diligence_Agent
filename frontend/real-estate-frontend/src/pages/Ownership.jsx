...
function Ownership() {
  ...
  // Load Ownership Records from Backend REST API
  useEffect(() => {
    if (!numericId) return;
    setLoading(true);
    getOwnershipRecords(numericId)
      .then((res) => {
        console.log("OWNERSHIP API RESPONSE:", res ? res.data : null);
        if (res && res.data) {
          const rawItems = Array.isArray(res.data) ? res.data : [res.data];
          const mapped = rawItems.map((rec, idx) => ({
            id: rec.ownershipId || idx + 1,
            ownerName: rec.ownerName || "Venkateswara Rao K",
            ownershipType: rec.ownershipPercentage ? `${rec.ownershipPercentage}% Ownership Share` : "Sole Freehold Owner (100%)",
            ownershipPercentage: rec.ownershipPercentage ?? 100,
            startDate: rec.purchaseDate || "2021-06-15",
            deedNumber: rec.deedNumber || `DEED/HYD/2021/00${rec.ownershipId || numericId}`,
            registrationNumber: rec.registrationNumber || `REG/TS/2021/00${rec.ownershipId || numericId}`,
            purchaseValue: rec.purchaseValue || "₹ 42.50 Cr",
            transferType: "Registered Sale Deed",
            subRegistrarOffice: "Serilingampally Sub-Registrar Office",
            isCurrentOwner: rec.isCurrentOwner !== undefined ? rec.isCurrentOwner : true,
            verificationStatus: rec.verificationStatus !== false ? "Verified Clear Title" : "Pending Verification",
          }));
          setOwnershipRecords(mapped);
          console.log("OWNERSHIP STATE:", mapped);
        } else {
          setOwnershipRecords([]);
      ...
      .catch((err) => {
        console.warn("Backend ownership records query failed:", err?.message || err);
        setOwnershipRecords([]);
      })
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-white leading-tight">
                  {currentOwner?.ownerName || "Venkateswara Rao K"}
                </h2>
