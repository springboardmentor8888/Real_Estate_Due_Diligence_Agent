...
function DueDiligenceReport() {
  ...
  const rawId = urlId || location.state?.propertyId || location.state?.property?.numericId || location.state?.property?.id || localStorage.getItem("active_property_id") || "1001";
  const cleanId = rawId.toString().replace(/\D/g, "") || "1001";
  ...
  const formatProperty = (dataObj) => {
    ...
    const numId = (raw.numericId || raw.propertyId || raw.id || cleanId).toString().replace(/\D/g, "") || "1001";
    const propTitle = raw.propertyName || raw.title || raw.name || "Gachibowli Tech Park Phase 2";
